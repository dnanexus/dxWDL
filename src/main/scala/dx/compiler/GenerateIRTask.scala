...
import dx.api.{DxApi, DxPath, DxUtils}
...
case class GenerateIRTask(dxApi: DxApi,
  ...
  def triageDockerImageFromValue(dockerValue: WdlValues.V): IR.DockerImage = {
    dockerValue match {
      case WdlValues.V_String(url) if url.startsWith(DxPath.DX_URL_PREFIX) =>
  ...
  def apply(task: TAT.Task, adjunctFiles: Option[Vector[Adjuncts.AdjunctFile]]): IR.Applet = {
    dxApi.logger.trace(s"Compiling task ${task.name}")
    ...
    val selfContainedSourceCode =
      WdlCodeGen(dxApi.logger, typeAliases, language).standAloneTask(cleanedTask)
