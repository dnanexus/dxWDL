...
case class JobMeta(homeDir: Path = DxWorkerPaths.HomeDir,
  ...
  val jsInputs: Map[String, JsValue] =
  ...
  private lazy val outputPath = homeDir.resolve(JobMeta.outputFile)
  ...
  private lazy val infoPath = homeDir.resolve(JobMeta.infoFile)
  private lazy val info = FileUtils.readFileContent(infoPath).parseJson.asJsObject.fields
