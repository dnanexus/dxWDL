...
case class VersionSupport(version: WdlVersion,
  ...
  def parse(sourceCode: FileSource): (TAT.Document, Map[String, WdlTypes.T_Struct]) = {
    Utils.parseSource(parser, sourceCode, fileResolver, regime, logger)
  }

  def parse(src: String): (TAT.Document, Map[String, WdlTypes.T_Struct]) = {
    parse(StringFileSource(src))
  }

  def parse(path: Path): (TAT.Document, Map[String, WdlTypes.T_Struct]) = {
...
object VersionSupport {
  def fromSource(
      ...
      dxApi: DxApi = DxApi.get,
      logger: Logger = Logger.get
  ): (TAT.Document, Map[String, WdlTypes.T_Struct], VersionSupport) = {
