...
case class Block(statements: Vector[GraphNode]) {
    ...
    def findCalls(statements1: Seq[GraphNode]) : Vector[CallNode] = {
...
object Block {
    ...
    private def isSubBlock(scope:GraphNode) : Boolean = {
        scope match {
    ...
    def splitIntoBlocks(children: Vector[GraphNode],
    ...
    //     Int n = add.result
    // }
    private def isReducible(scope: GraphNode) : Boolean = {
        if (!isSubBlock(scope))
            return false
    ...
    def findTrunkTop(stmt: GraphNode) : GraphNode = {
